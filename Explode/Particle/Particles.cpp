...
void ParticleSystem::updateAll(void){
	...
	for(int i = 0; i < NUMBER_OF_PARTICLES; i++){
		if(particles[i].life_time == 0)
			...
			if(particles[i].surface_translation_factor > 20)
				//particles[i].expand_flag = false;
				EXPAND_FLAG = false;
			else if(particles[i].surface_translation_factor < 0.03)
				//particles[i].expand_flag = true;
				EXPAND_FLAG = true;
			
			if(EXPAND_FLAG)
			//if(particles[i].expand_flag)
